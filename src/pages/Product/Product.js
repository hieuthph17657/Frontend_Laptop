...
const Product = () => {
  ...
  const search = () => {
    ...
    tableParams.pagination.search2 = searchStatus ? searchStatus : "";
    tableParams.pagination.search3 = searchPrice ? searchPrice : "";
    tableParams.pagination.search4 = searchPn ? searchPn : "";
  ...
  const clearSearchForm = () => {
    ...
    tableParams.pagination.search4 = "";
    tableParams.pagination.current = 1;
    tableParams.pagination.pageSize = 10;
    load();
    setSearchProductKey("");
...
const loadDataCategory = () => {
  ...
  const handleImport = async (data, index) => {
    ...
    console.log("productImp",product);
      const responseImp=await fetch("http://localhost:8080/api/staff/products", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + localStorage.getItem("token"),
      ...
      // .then((response) => {
        ...
        const response=await responseImp.json();
        if (response.data) {
  ...
  const importExcel =(e) => {
    ...
    for (let i = 0; i < dataImport.length; i++) {
      const data = dataImport[i];
      const missingFields = [];
      for (const field of requiredFields) {
        if (!data[field] &&data[field]!==0 || (data[field]+"").trim() === "") {
    ...
    if (missingData.length > 0) {
      //console.log("Các đối tượng data sau có trường bị trống:");
      missingData.forEach(item => {
        var mesError=
          `Dòng ${item.line}. Trường bị trống: ${item.missingFields.join(
            ", "
          )}`
        ;
        notifyError(mesError);
      });
    }else{
      dataImport
