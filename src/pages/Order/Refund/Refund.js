...
const Refund = () => {
  ...
  const confirmCheckBox = () => {
    ...
    Modal.confirm({
      ...
      content: `Bạn chắc chắn muốn hoàn tiền những đơn hàng này chứ?`,
      okText: "Có",
      cancelText: "Không",
      okType: "primary",
      onOk: () => {
  ...
  const handleOk = (record, check) => {
    ...
    if (check == true) {
      Modal.confirm({
        ...
        content: `Bạn có muốn xác nhận hoàn tiền đơn hàng ${record?.id} 
  ...
  const handleTableChange = (pagination, filters, sorter) => {
    ...
    tableParams.pagination.searchStatusOrder = "DA_HUY";
    tableParams.pagination.searchPayment = "";
    tableParams.pagination.searchMoney = "";
    loadDataOrder();
  };
  ...
  const loadDataOrder = () => {
    setLoading(true);
    fetch(
      `http://localhost:8080/api/staff/orderFinishMoney?${qs.stringify(
  ...
  const handleConfirm = (isPut, record) => {
    ...
    fetch(`http://localhost:8080/api/staff/orders/refund`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + localStorage.getItem("token"),
    ...
    <div>
      <div className="row">
        ...
        <div className="col-11">
          <h4 className="text-danger fw-bold">Đơn hoàn tiền</h4>
        </div>
      </div>
      <div
        className="row"
