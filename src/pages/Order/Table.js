...
function CreateOrderAdmin() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (data) => {
        return (
          <>
            {data.quantity < data.quantityProduct ? (<InputNumber
              style={{ width: "50px" }}
              min={1}
            ...
            />) : (<InputNumber
              ...
              defaultValue={data.quantityProduct}
              onChange={(event) => onChangeInputNumberCart(data, event)}
            />)}
          </>
        );
        ...
        try {
          fetch("http://localhost:8080/api/auth/orders", {
            ...
            body: JSON.stringify({
              ...
              phone: order.phone,
              status:
                typeOrder === "TẠI CỬA HÀNG" && checkConfirm == true
                  ? "DA_NHAN"
                  : checkConfirm == true
                  ? "CHO_LAY_HANG"
                  : "CHO_XAC_NHAN",
  ...
  const onChange = (value) => {
    ...
    if(valueProvince == null || valueProvince == undefined) {
     
      array?.forEach(item => { 
        if(item.ProvinceID === value) {
        
          setValueProvince(item.ProvinceName);
        }
      })
    }  
    setProvinceID(value);
  ...
  const onChangeDistricts = (value) => {
    ...
    district?.forEach(item => {
      if(item.DistrictID == value) {
        setValueDistrict(item.DistrictName);
      }
    });
    setDistrictId(value);
    loadDataWard(value);
  ...
  const onChangeWards = (value) => {
    ...
    Ward?.forEach(item => {
      if(item.WardCode == value) {
        setValueWard(item.WardName);
      }
    })
    setWardCode(value);
              ...
              <div className="col-6">
                ...
                >
                  <Select placeholder="Hình thức thanh toán">
                    <Option value="Thanh toán tiền mặt">Bằng tiền mặt</Option>
