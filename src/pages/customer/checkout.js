...
function Checkout() {
  ...
  const [payment, setPayment] = useState("NGAN_HANG");
  ...
  const createOrder = () => {
    console.log("cartCheckout",JSON.parse(localStorage.getItem("cartCheckout")));
    console.log("products",products?products:null);
    (JSON.parse(localStorage.getItem("cartCheckout"))?JSON.parse(localStorage.getItem("cartCheckout")):[]).forEach((item) => {
      const found = products.find((c) => c.id === item.id);
      console.log("item.id",item.id);
      if (found) {
        console.log("found",found);
        if (found.quantity < item.quantity) {
          toastError(`Số lượng sản phẩm ${found.name} trong giỏ hàng lớn hơn số lượng trong kho(số lượng trong kho: ${found.quantity})!`);
        }else{
          var districtName="";
    var wardName="";
    var provinceName="";
    ...
    if ((JSON.parse(localStorage.getItem("cartCheckout"))?JSON.parse(localStorage.getItem("cartCheckout")):[]).length<=0) {
      ...
      if (type == 1) {//Tai nha
        ...
        if (provinceName==undefined||provinceName==null||provinceName=="") {
          ...
          if (parseInt(getTotal(carts)) > 200000000) {
            ...
            if (payment === "DAT_COC") {
              ...
              // } else {
                diachi =(address.replace(',', '-')).trim()+", "+wardName+", "+districtName.trim()+", "+provinceName.trim();
                  // address.replace(',', '-') +
                  // ", " +
                  // valueWard +
                  // ", " +
        ...
        localStorage.setItem("districtName",districtName.trim());
        localStorage.setItem("provinceName",provinceName.trim());
  ...
  const changePayment = (event) => {
    ...
    if (event.target.value === "DAT_COC"||event.target.value === "VN_PAY") {
          ...
          <div>
            {type === "1" ? (
              ...
              <div>
                <div className="row ck-content">
                  <div className="col-12" style={{ paddingLeft: "20px" }}>
                    <p style={{ fontWeight: "600" }}>Địa chỉ nhận hàng tại cửa hàng</p>
                    <div>
