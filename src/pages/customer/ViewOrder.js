...
function ViewOrder() {
  ...
  const getData = () => {
    ...
    if (param === "00") {
      ...
      let wardName=localStorage.getItem("wardName");
      ...
      // } else {
      console.log("vào đầu");
      dc = address + ", " + wardName + ", " + districtName + ", " + provinceName;
      //}
...

    {
      ...
      render: (id, data) => {
        ...
        if (data.status === "CHO_XAC_NHAN") {
          ...
          return (
            <>
            <Button shape="round" onClick={() => confirmCheckBox(data)} danger>Nhận hàng</Button>
              <EyeOutlined
                style={{ fontSize: "20px", marginLeft: "25%" }}
                onClick={() => {
                  showModalData(data.id);
  ...
  const confirmCheckBox = (record) => {
   
    Modal.confirm({
      icon: <CheckCircleOutlined />,
      title: "Nhận hàng ",
      content: `Bạn có chắc chắn nhận hàng không?`,
      okText: "Có",
      cancelText: "Không",
      okType: "primary",
      onOk: () => {
  ...
  const handleConfirm  = (record)  => {
    ...
    fetch(`http://localhost:8080/api/auth/orders/confirm/2`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + localStorage.getItem("token"),
      ...
      body: JSON.stringify(dataOrder),
        ...
        getData();
        toastSuccess("Nhận hàng thành công !");
    });
  }
